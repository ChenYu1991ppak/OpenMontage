...
|----------|-----------|-----------------|
| `tts_selector` | All tools with `capability="tts"` (ElevenLabs, Google TTS, OpenAI, Piper, IndexTTS25 local) | `registry.get_by_capability("tts")` |
...
| **Audio** | `elevenlabs`, `music`, `sound-effects`, `acestep`, `text-to-speech`, `index-tts` (local zero-shot cloning, emotion_text), `setup-api-key` |
...
| **Media editing** | `video-edit`, `video-download`, `video-understand`, `image-understand`, `video-toolkit`, `visual-style` |
